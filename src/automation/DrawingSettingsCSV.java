	...
	public static void saveSettings(WebDriver driver, WebDriverWait wait) {
	    ...
	    for (int i = 0; i < tileHrefs.size(); i++) {
	        ...
	        try {
	        	...
	        	for (Map.Entry<String, Map<String, String>> tabEntry : settingsByTab.entrySet()) {
	        	    String tabName = tabEntry.getKey();
	        	    Map<String, String> tabSettings = tabEntry.getValue();

	        	    for (Map.Entry<String, String> setting : tabSettings.entrySet()) {
	        	        String key = tabName + " | " + setting.getKey();  // Prefix key with tab name
	        	        row.put(key, setting.getValue());
	        	        allHeaders.add(key);  // Add to CSV header list
	        	    }
	        	}
